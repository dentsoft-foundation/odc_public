...
# ModalOperator
class CutMesh_Polytrim(
    Polytrim_States280, Polytrim_UI_Init280, Polytrim_UI_Tools, Polytrim_UI_Draw280
):
    ...
    def start(self):
        ...
        self.variable_1 = BoundFloat(
            """options['variable_1']""", min_value=0.5, max_value=15.5
        )
        ...
        self.net_ui_context = self.NetworkUIContext(
            source_ob, geometry_mode=self.destructive
        )
        ...
        self.sketcher = self.SketchManager(
            self.input_net, self.spline_net, self.net_ui_context, self.network_cutter
        )
        self.grabber = self.GrabManager(
            self.input_net, self.net_ui_context, self.network_cutter
        )
        self.brush = None
        ...
        self.polytrim_render = SplineNetworkRender(
            self.spline_net, self.network_cutter, render_opts
        )
