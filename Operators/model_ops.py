...
def Second_separate_method():
    ...
    resulting_parts = (
        filter_loose_parts()
    )  # all visible objects are selected after func
        ...
        if bpy.context.selected_objects == []:
            ...
            for v in selected_verts:
                ...
                v.co = obj_mx.inverted() @ Vector(
                    (global_v_co[0], global_v_co[1], min_z - base_height)
                )
            # Store Base location :
            context.scene.ODC_modops_props.base_location_prop = (
                0,
                0,
                min_z - base_height,
            )
...
class OPENDENTAL_OT_solid_hollow_models(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.context.selected_objects == []:
            ...
            Model_solid_base.matrix_world = (
                unflip_matrix @ Model_solid_base.matrix_world
            )
            ...
            print(
                f"Making model solide base and model hollow base finished in {finish-start} second(s)"
            )
    ...
    def execute(self, context):
        ...
        try:
            if not "Calibri" in bpy.data.fonts:
                ...
                bpy.ops.font.open(
                    filepath="C:\\WINDOWS\\Fonts\\calibri.ttf", relative_path=True
                )
                bpy.ops.font.open(
                    filepath="C:\\WINDOWS\\Fonts\\calibrib.ttf", relative_path=True
                )
                bpy.ops.font.open(
                    filepath="C:\\WINDOWS\\Fonts\\calibrii.ttf", relative_path=True
                )
                bpy.ops.font.open(
                    filepath="C:\\WINDOWS\\Fonts\\calibriz.ttf", relative_path=True
                )
        except Exception:
            pass
